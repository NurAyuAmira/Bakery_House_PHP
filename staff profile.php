...
<html lang="en">
<head>
  <title>Account Information - Staff</title>
  <meta charset="utf-8">
        ...
        <div class="container-fluid px-3">
            ...
            <div class="collapse navbar-collapse justify-content-end" id="collapsibleNavbar" style="font-size:x-large;font-weight: bold;">
                <ul class="navbar-nav text-uppercase">
                    <li class="nav-item">
                        <a class="nav-link px-3" href="staff order.php">Order</a>
                    </li>
                    <li class="nav-item">
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-dark dropdown-menu-end" style="font-size:x-large">
                                <li><a class="dropdown-item" href="staff%20profile.php">My Profile</a></li>
                                <li><hr class="dropdown-divider"></li>
                                <li><a class="dropdown-item" href="staff%20signout.php">Sign Out</a></li>
                            </ul>
                        </div>
                    </li>
                </ul>
        ...
        <div class="content">
            <div class="tab-content">
                <div id="manage" class="tab-pane active">
                    ...
                    $aDetails = mysqli_query($connect,"select * from staff where Staff_ID = '$sid'");
                    $row = mysqli_fetch_assoc($aDetails);
                ?>
                    <form action="" method="post" class="bg-white rounded-3" onsubmit="return(validateForm());">
                        <fieldset disabled id="staffform">
                            <div class="mt-3 d-flex justify-content-between">
                                <div>
                                    <label for="email" class="form-label"><i class="bi bi-envelope-fill"></i>&nbsp;Email :</label>
                                    <h6 name="email"><?php echo $row['Staff_Email']?></h6>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    <div class="mt-3">
                                        ...
                                        <input type="text" class="form-control" id="fname" placeholder="Enter Your first name" name="fname" required value="<?php echo $row['Staff_First_Name']?>">
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    <div class="mt-3">
                                        <label for="lname" class="form-label"><i class="bi bi-person-circle"></i>&nbsp;Last Name :</label>
                                        <input type="text" class="form-control" id="lname" placeholder="Enter Your last name" name="lname" required value="<?php echo $row['Staff_Last_Name']?>">
                                    </div>
                                </div>
                            </div>
                            <div class="mt-3">
                            ...
                            <div class="mt-3">
                                ...
                                <input type="tel" class="form-control" id="phone" placeholder="Enter Your phone number" name="phone" required value="<?php echo $row['Staff_Contact_No']?>" onchange="checkPhone();">
                            </div>
                                       <div class="mt-3">
                                <label for="address" class="form-label"><i class="bi bi-geo-alt-fill"></i>&nbsp;Address :</label>
                                <input id="address" placeholder="Enter Your Address" name="address" type="text" class="form-control" required value="<?php echo $row['Staff_Address']?>">
                                <br>
                                <div id="mt-3">
                                    <div class="row">
                                        <div class="col-md-4">
                                            <label for="city" class="form-label">City</label>
                                            <input class="form-control" id="city" name="city" required value="<?php echo $row['Staff_Address_City']?>">
                                        </div>
                                        <div class="col-md-4"> 
                                            <label for="state" class="form-label">State</label>
                                             <input class="form-control" id="state" name="state" required value="<?php echo $row['Staff_Address_State']?>">
                                        </div>
                                        <div class="col-md-4">
                                            <label for="postcode" class="form-label">Postcode</label>
                                            <input class="form-control" id="postcode" name="postcode" pattern="[0-9]{5}" required value="<?php echo $row['Staff_Address_Postcode']?>">
                                        </div>
                                    </div>
                                </div>
                            </div> 
...
<script>
        document.getElementById('editbtn').onclick = function()
        {
            ...
            document.getElementById('staffform').disabled = false;
        }
        ...
        document.getElementById('backbtn').onclick = function()
        {
            ...
            document.getElementById('staffform').disabled = true;
        }
...
<?php 
    if(isset($_POST['changebtn']))
    {
        ...
        $accPW = $row['Staff_Password'];

        if($currPW == $accPW)
        {
            mysqli_query($connect,"update staff SET Staff_Password = '$newPW' where Staff_ID LIKE '$sid'");
?>
            <script>
...
<?php
    if(isset($_POST['savebtn']))
    {
        ...
        mysqli_query($connect,"update staff SET Staff_First_Name = '$fname', Staff_Last_Name = '$lname', Staff_Gender = '$gender', Staff_Contact_No = '$phone' ,Staff_Address = '$address', Staff_Address_Postcode = '$postcode', Staff_Address_City = '$city',Staff_Address_State = '$state'where Staff_ID LIKE '$sid'");
?>
        <script>
