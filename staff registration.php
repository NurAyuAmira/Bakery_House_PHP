        ...
        <div class="row vh-100">
            ...
            <div class="col text-center mt-5 pt-5">
                ...
                <div class="container shadow-sm my-5 bg-light py-4" style="max-width: 80%;">
                    <h3><i class="bi bi-people-fill"></i>&nbsp Staff Registration</h3>
...
if(isset($_POST["registerbtn"]))
{
    ...
    $checkresult = mysqli_query($connect,"select * from staff where Staff_Email = '$email'");
    if(mysqli_num_rows($checkresult)==0)
    {
        $checkid = "select * from staff ORDER BY Staff_ID DESC LIMIT 1";
        $checkresult = mysqli_query($connect,$checkid);
        if(mysqli_num_rows($checkresult)>0)
        {
            if($row = mysqli_fetch_assoc($checkresult))
            $lid = $row['Staff_ID'];
            $getIDNum = str_replace("S","",$lid);
            $idPlus = $getIDNum+1;
        ...
        mysqli_query($connect,"insert into staff(Staff_ID,Staff_First_Name,Staff_Last_Name,Staff_Email,Staff_Password,Staff_Address,Staff_Address_Postcode,Staff_Address_City,Staff_Address_State, Staff_Gender,Staff_Contact_No)
        values('$StaffID','$fname','$lname','$email','$password','$address','$postcode','$city','$state','$gender','$phone')");
?>
        <script>
            alert("Account Registration for Staff Is Successful. Please Login Your Account.");
